    ...
    def setup_model(self):
        with SetVerbosity(self.verbose):
            ...
            assert issubclass(self.policy, ActorCriticPolicy), (
                "Error: the input policy for the GAIL model must be "
            ...
            with self.graph.as_default():
                ...
                self.reward_giver = RewardClassifier(
                ...
                with tf.variable_scope("Adam_mpi", reuse=False):
                    ...
                    self.d_adam = MpiAdam(
                        self.reward_giver.get_trainable_variables(), sess=self.sess
                    )
                    self.d_adam.sync()
                    self.vfadam.sync()
                ...
                while True:
                    ...
                    for k in range(self.g_step):
                        ...
                        with self.timed("computegrad"):
                            ...
                            run_metadata = (
                                tf.RunMetadata() if self.full_tensorboard_log else None
                            )
